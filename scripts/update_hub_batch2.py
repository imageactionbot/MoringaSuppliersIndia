# -*- coding: utf-8 -*-
"""Patch articles/index.html and sitemap for batch-2 business articles."""
...
def load_batch2():
    path = Path(__file__).resolve().parent / "moringa_business_article_batch2.py"
    spec = importlib.util.spec_from_file_location("batch2", path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod.BATCH2_ARTICLES


def main() -> None:
    batch2 = load_batch2()
    text = INDEX.read_text(encoding="utf-8")
    text = text.replace('<div class="phs-value">39</div>', '<div class="phs-value">58</div>', 1)
    ...
    for a in batch2:
        new_urls.append(
            ...
            f"<lastmod>2026-05-05</lastmod><changefreq>monthly</changefreq><priority>0.86</priority></url>"
        )
    blob = "\n".join(new_urls) + "\n"
    ...
    print("sitemap: appended", len(batch2), "article URLs")
    ...
    for p in (ROOT / "articles").glob("*.html"):
        ...
        if "(39 articles)" in t:
            p.write_text(t.replace("(39 articles)", "(58 articles)"), encoding="utf-8")
            n += 1
